...
sub ProcessMenu {
	...
	my $fLargest = glGetFloatv_p(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT);
        glTexParameterf_p(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, $fLargest);
    } elsif($value == 7) {
...
if(gltExt::gltIsExtSupported('GL_EXT_texture_filter_anisotropic')) {
    print "GL_EXT_texture_filter_anisotropic - SUPPORTED!\n";
} else {
    print "GL_EXT_texture_filter_anisotropic - NO WORK this extension!\n";
}
