...
my $iMethod = 0; 	# Without display lists
    # Display list identifiers
my ($sphereList, $groundList, $torusList);
...
sub RenderScene {
	...
	#измеряем количество кадров в секунду - инициализация
    my ($seconds, $microseconds);
    if($iFrames == 0) {
	($seconds, $microseconds) = gettimeofday;
	$last = $seconds + 0.000001* $microseconds;
	$iFrames++;
    }   
	...
	#измеряем количество кадров в секунду - подсчет
    $iFrames++;
    if($iFrames == 101) {
	($seconds, $microseconds) = gettimeofday;
	my $cur = $seconds + 0.000001* $microseconds;
	my $fps = 100.0/(abs($cur-$last) || 1);
	print "second:", abs($cur-$last), "\n" ;
	my $title;
	if($iMethod) {
            $title = "OpenGL SphereWorld without Display List $fps fps";
	} else {
            $title = "OpenGL SphereWorld with Display List $fps fps";
	}
