...
my @textureObjects;
my @CubeFacesFiles = ('right.tga', 'left.tga', 'up.tga', 'down.tga', 'backward.tga', 'forward.tga');
my @cube = (
...
sub SetupRC {
	...
	#Textures load
    glEnable(GL_TEXTURE_2D);
	#количество текстур будет равно количеств у TextureFiles + 1 CUBE_MAP
    @textureObjects = glGenTextures_p($#TextureFiles+1+1);
    ...
    foreach my $i (0..$#CubeFacesFiles) {
        my $name_f =$CubeFacesFiles[$i];
        $img = new OpenGL::Image(source=>$name_f);
        ($w_img,$h_img) = $img->Get('width','height');
        ($def_ifmt, $def_fmt,$def_type) = $img->Get('gl_internalformat', 'gl_format','gl_type');
	glTexImage2D_c($cube[$i],  0, $def_ifmt, $w_img, $h_img, 0, $def_fmt, $def_type, $img->Ptr() );
    }
