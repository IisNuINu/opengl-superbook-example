...
my $face_indicies = \@{ship_data::face_indicies};
my $normals       = \@{ship_data::normals};
my $textures      = \@{ship_data::textures};
my $vertices      = \@{ship_data::vertices};

#Textures
my ($w_img, $h_img, $img);
my ($def_ifmt, $def_fmt,$def_type);
my @TextureFiles = ('stone.tga');
my @textureObjects;

use constant   SHIP_TEXTURE => 0;
